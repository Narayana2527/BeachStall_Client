...
import { Search, Flame, ShoppingBag, Loader2, ChefHat } from 'lucide-react';
...
const ModernMenu = () => {
  ...
  const handleAddToCart = (item) => {
    ...
    if (activeTab === 'catering') {
      navigate('/catering-booking', { state: { product: item } });
      return;
    }
    ...
    // The toast alert is handled inside this function within CartContext.js
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 transition-colors duration-300">
      ...
      <div className="fixed top-[80px] left-0 right-0 z-[47] h-[70px] bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-zinc-100 dark:border-zinc-900 flex items-center justify-center px-6">
        <div className="relative w-full max-w-xl">
          ...
          <input 
            ...
            className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl py-3 pl-12 pr-4 text-sm outline-none focus:ring-2 focus:ring-orange-500/20"
            value={searchQuery}
      ...
      {/* CATEGORY NAV */}
      <nav className="fixed top-[150px] left-0 right-0 z-[40] bg-white dark:bg-zinc-950 border-b border-zinc-100 dark:border-zinc-900">
        <div className="max-w-7xl mx-auto flex overflow-x-auto no-scrollbar py-4 px-4 gap-3 justify-start md:justify-center">
          {categories.map((cat) => (
            <button
              key={cat.id}
      ...
      <main className="max-w-7xl mx-auto px-6 pt-[250px] pb-24">
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-3">
            <span className="h-[2px] w-8 bg-orange-500" />
            <span className="text-orange-500 font-mono text-[10px] tracking-[0.4em] uppercase font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {filteredItems.map((item) => (
            <div key={item._id || item.id} className="group flex flex-col justify-between border-t border-zinc-100 dark:border-zinc-900 pt-8 hover:-translate-y-1 transition-transform">
              <div>
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-2">
                    {activeTab === 'catering' ? <ChefHat size={14} className="text-orange-500" /> : <Flame size={14} className="text-orange-600" />}
                  ...
                  <div className="flex flex-col items-end">
                    <span className="text-xl font-black italic text-orange-600">
                      ₹{item.price}{activeTab === 'catering' && '/pp'}
                    </span>
                  </div>
                </div>
                <h3 className="text-xl font-bold uppercase mb-2">{item.name}</h3>
                <p className="text-sm text-zinc-500 italic line-clamp-2">{item.description}</p>
              </div>
              
              <button onClick={() => handleAddToCart(item)} className="relative mt-8 overflow-hidden rounded-xl bg-gray-900 dark:bg-zinc-100 py-3 group/btn">
                <div className="absolute inset-0 bg-orange-500 translate-y-full transition-transform group-hover/btn:translate-y-0" />
                <div className="relative flex items-center justify-center gap-3 text-xs font-black text-white dark:text-zinc-900 group-hover/btn:text-white uppercase tracking-widest">
                  <ShoppingBag size={16} />
                  <span>{activeTab === 'catering' ? 'View Catering Details' : 'Add To Cart'}</span>
                </div>
              </button>
            </div>
          ))}
